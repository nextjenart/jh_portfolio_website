...
$oi_userid = $current_user->ID;
	...
	if(!empty( $oi_username)){
	$output = $oi_username;
	}else {$output = __('Customer', "orangeidea");};
	return $output;
...
function theme_port_i( $atts, $content = null)
{
	...
	if($content) { $output .= '<p>'.theme_remove_autop(stripslashes($content)).'</p>'."\n"; }
	$output .= '<div class="oi_short_port_holder">'.theme_port_loop_i($show_posts, $header, $cat).'</div>';
	return $output;
...
function theme_port_loop_i($show_posts, $header, $cat)
{
	...
	while ($query->have_posts()) { $query->the_post();
		...
		echo '<div class="oi_strange_portfolio_item oi_col-md-3">'."\n";
			echo '<a href="'.get_the_permalink().'">'."\n";
			echo '<img class="img-responsive" style="width:100%;" src="'. $large_image_url[0].'" alt="" />'."\n";
			echo '<div class="oi_mask" style="background:'.get_post_meta($post_id, 'port-bg', 1).'">'."\n";
				echo '<h4 class="oi_port_title" style="color:'.get_post_meta($post_id, 'port-text-color', 1).'">'. get_the_title($post_id).'</h4>'."\n";
				echo '<div class="oi_small_descr" style="color:'.get_post_meta($post_id, 'port-text-color', 1).'">'.get_post_meta($post_id, 'port-descr', 1).'</div>'."\n";
				echo '<div class="oi_port_cats" style="color:'.get_post_meta($post_id, 'port-text-color', 1).'">'."\n";
...
function vbg_f($atts, $content = null) {
	...
	'webm_format' => 'http://orange-idea.com/video/demo.webm',
	'ogv_format' => 'http://orange-idea.com/video/demo.ogv',
	'mp4_format' => 'http://orange-idea.com/video/demo.mp4',
	), $atts));
	...
	$code = '
			<div class="oi_video_bg" style="background:#fdfdfd;">
...
function vbgg_f($atts, $content = null) {
	extract( shortcode_atts( array(
	'image'=> get_template_directory_uri().'/assets/img/no_image.png',
	'webm_format' => 'http://orange-idea.com/video/mb.webm',
	'ogv_format' => 'http://orange-idea.com/video/mb.ogv',
	'mp4_format' => 'http://orange-idea.com/video/mb.mp4',
	), $atts));
	...
	$code = '
			<div class="oi_video_bg vid_bgg" style="background:#fdfdfd;">
				<div class="vid_bg_content">
...
function c_break_f($atts, $content = null) {
	...
	<div class="'.$extraclass.'" style="height: '.$h.'; margin-top:'.$m_t.'; margin-bottom:'.$m_b.'; padding-top:'.$p_t.'; padding-bottom:'.$p_b.'; width: 100%; background-color: '.$bg.'; background-image:url('.$image_done1.'); background-position:center center; border-top: 1px solid '.$border.'; border-bottom: 1px solid '.$border.';">
		<div>
			<div class="oi_break container"><span style="color:'.$color.'">'.do_shortcode($content).'</span></div>
		</div>
	</div>
	...
	<div class="'.$extraclass.'" style="height: '.$h.'; margin-top:'.$m_t.'; margin-bottom:'.$m_b.'; padding-top:'.$p_t.'; padding-bottom:'.$p_b.'; width: 100%; background-color: '.$bg.'; background-image:url('.$image_done1.'); background-position:center center;  border-top: 1px solid '.$border.'; border-bottom: 1px solid '.$border.'; ">
		<div class="oi_break" style="padding:0px 20px;">
...
function c_break_half_paralax_break_f($atts, $content = null) {
	...
	if($fixed == "0") $code = '</div></div></div></div>
	<div class="'.$lastclass.' oi_half_paralxx" style=" width: 100%; margin-top:'.$first1.'; margin-bottom: '.$last1.' !important;">
	'.$laststyle.'
		<div style="height: '.$h.';" class="oi_half_paralxx_cont">
			<div class="oi_break">
...
function vc_oi_gallery_f($atts, $content = null) {
	extract(shortcode_atts( array(
	'images'=> array(''),
	), $atts));
	
	 $attachments = explode(',', $images);
	 $code = '<div id="port_slider" class="flexslider oi_flex_loading oi_vc_gal">
				<ul class="slides">';
...
add_shortcode('vc_oi_gallery', 'vc_oi_gallery_f');
...
function vc_price_f($atts, $content = null) {
	...
	if ($price_spec == '1'){ $spec = 'spec';} else{ $spec = '';};
	 ...
	 $code = 
	 '<div class="oi_price_holder '.$spec.'">
	 	<div class="oi_price_head_holder" style="background-color:'.$price_head_bg.'">
			<div class="oi_price_head">
				<h4 style="color:'.$price_title_color.' !important">'.$price_title.'</h4>
			</div>
			<div class="oi_price_price">
		...
		<div class="unstyled oi_price_ul">
		'.do_shortcode($content).'
		</div>
		<div class="oi_price_order">
			<a class="btn oi_submit oi_reverse_btn" href="'.$url_on_button.'">'.$text_on_button.'</a>
		</div>
	 </div>
	 ';
...
function box_f($atts, $content = null) {
	...
	elseif($columns == '1'){$columns1 = 'span12';}
	 ...
	 $code = '<div class="'.$columns1.'">
	 			<div class="oi_box" style=" background:'.$bg.'; border:1px solid '.$border.'; color:'.$color.' !important;"><div class="icon_holder" style="background:'.$icon_bg.'; "><img class="oi_box_icon" src="'.$icon.'" alt=""></div>'.do_shortcode($content).'</div>
			</div>
			 ';
...
function vc_box_f($atts, $content = null) {
	...
	'color' => '',
	'border' => '#ededed',
	'icon'=> 'http://html.orange-idea.com/splendor/wp-content/themes/splendor/assets/img/plane.png',
	'title'=> 'The Box Title',
	), $atts));
	
	
	
	 $code = '<div class="oi_box" style=" background:'.$bg.'; border:1px solid '.$border.'; color:'.$color.' !important;"><div class="icon_holder" style="background:'.$icon_bg.'; "><img class="oi_box_icon" src="'.$icon.'" alt=""></div><h3 clas="small_width" style="color:'.$color.' !important;">'.$title.'</h3>'.do_shortcode($content).'</div>';
...
function vc_box_ii_f($atts, $content = null) {
	...
	'title'=> 'The Box Title',
	 ...
	 $code = '<div class="oi_box_ii" style=" background:'.$bg.'; border:1px solid '.$border.'; color:'.$color.' !important;"><div class="icon_holder_ii" style="background:'.$icon_bg.'; "><img class="oi_box_icon" src="'.$icon.'" alt=""></div><h3 clas="small_width" style="color:'.$color.' !important;">'.$title.'</h3>'.do_shortcode($content).'</div>';
...
function vc_box_iii_f($atts, $content = null) {
	...
	'title'=> 'The Box Title',
	 ...
	 $code = '<div class="oi_box_iii">
	 			<div class="icon_holder_iii">
					<img class="oi_box_icon_iii" src="'.$icon.'" alt="">
					<h3>'.$title.'</h3>
				</div>
...
function vc_box_iv_f($atts, $content = null) {
	...
	'bgcolor'=> '#000',
	 ...
	 $code = '<div class="oi_box_iv">
				<div class="icon_holder_iv" style="background:'.$bgcolor.'">
					<span class="oi_box_icon_iv colored fa '.$icon.'" style="color:'.$iconcolor.'"></span>
				</div>
				<div class="cont_holder_iv"><h4>'.$title.'</h4>'.do_shortcode($content).'</div>
			 </div>
			 <div class="clearfix"></div>';
...
function testimonial_i_f($atts, $content = null) {
	...
	'date' => '',
	'border' => '#ededed',
	'image'=> 'http://html.orange-idea.com/splendor/wp-content/uploads/2013/08/proff.png',
	'image_style'=> '1',
	'title'=> 'Some Author',
	'rating'=> '1',
		...
		<div class="row">
			...
			<div class="col-md-9">
				<div class="testimonial_i_holder"  '.$margin_zero.'>
					<h3 class="small_width colored">'.$title.'</h3>
					...
					<div class="row">
						...
						<div class="blank_rating"> </div>
							<div class="star_rating" style="width:'.$rating1.'"></div>
						</div>
						<div class="col-md-6">
							<div class="t_comapny">
...
function ois_f($atts, $content = null) {
	...
	$code = '
            <div class="oi_slider">'.do_shortcode($content).'</div>
			
	';
	return $code;
}

add_shortcode('oi_slider', 'ois_f');
...
function ois_ii_f($atts, $content = null) {
	...
	$code = '
            <div class="oi_slider_ii">'.do_shortcode($content).'</div>
			
	';
	return $code;
}

add_shortcode('oi_slider_ii', 'ois_ii_f');
...
function mes_f($atts, $content = null) {
	extract( shortcode_atts( array(), $atts));
	global $woocommerce;
...
function testimonialrotator_f($atts, $content = null) {
	...
	$code = '
			<div class="sidebar oi_testimonial_rotarot">
				<div class="bxslider">
					'.do_shortcode($content).'
				</div>
			</div>
